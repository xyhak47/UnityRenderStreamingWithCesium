...
public class CustomCesiumCameraController : MonoBehaviour
{
    ...
    private bool ConfigureInputsFinished = false;

    public void ConfigureInputs(InputActionMap map)
    {
        //InputActionMap map = new InputActionMap("Cesium Camera Controller");

        lookAction = map.AddAction("cesium_look", binding: "<Mouse>/delta");
        lookAction.AddBinding("<Gamepad>/rightStick").WithProcessor("scaleVector2(x=15, y=15)");

        //lookAction = map.AddAction("cesium_look", InputActionType.Value);
        //lookAction.AddBinding(path: "<Mouse>/delta").WithInteraction("hold(duration=0.75)");


        moveAction = map.AddAction("cesium_move", binding: "<Gamepad>/leftStick");
        ...
        moveUpAction = map.AddAction("cesium_moveUp");
        ...
        speedChangeAction = map.AddAction("cesium_speedChange", binding: "<Mouse>/scroll");
        speedChangeAction.AddCompositeBinding("Dpad")
            .With("Up", "<Gamepad>/rightShoulder")
            .With("Down", "<Gamepad>/leftShoulder");

        speedResetAction = map.AddAction("cesium_speedReset", binding: "<Mouse>/middleButton");
        speedResetAction.AddBinding("<Gamepad>/buttonNorth");

        toggleDynamicSpeedAction =
            map.AddAction("cesium_toggleDynamicSpeed", binding: "<Keyboard>/g");
