...
public class CustomFreeCamera : MonoBehaviour
{
    ...
    public void RegisterInputs(InputActionMap map)
    {
#if USE_INPUT_SYSTEM
            //var map = new InputActionMap("Free Camera");

            lookAction = map.AddAction("city_look", binding: "<Mouse>/delta");
            moveAction = map.AddAction("city_move", binding: "<Gamepad>/leftStick");
            speedAction = map.AddAction("city_speed", binding: "<Gamepad>/dpad");
            yMoveAction = map.AddAction("city_yMove");
